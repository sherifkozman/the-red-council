...
# Added polling integration tests

def test_polling_adds_events_with_sdk_session_id(monkeypatch):
    import src.ui.components.event_stream as es

    es.st.session_state["sdk_session_id"] = "token-123"
    es.st.session_state["api_base_url"] = "http://localhost:8000"
    es.st.session_state["sdk_auth_token"] = "token"
    ...
    monkeypatch.setattr(es, "poll_events_from_api_sync", lambda **kwargs: (events, 2))
    monkeypatch.setattr(es.time, "sleep", lambda *_args, **_kwargs: None)
...
def test_polling_handles_error(monkeypatch):
    import src.ui.components.event_stream as es
    ...
    monkeypatch.setattr(es, "poll_events_from_api_sync", _raise)
    monkeypatch.setattr(es.time, "sleep", lambda *_args, **_kwargs: None)

    es.render_event_stream()
    state = es.get_stream_state()
    assert state.connection_status == es.ConnectionStatus.ERROR
