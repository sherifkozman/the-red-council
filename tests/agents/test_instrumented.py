...
def test_wrap_memory_access(instrumented_agent):
    instrumented_agent.wrap_memory_access(
        MemoryAccessOperation.READ, "user_name", "alice"
    )
    assert len(instrumented_agent.events) == 1
    event = instrumented_agent.events[0]
...
def test_detect_divergence_no_actions(instrumented_agent):
    """Test divergence detection returns None when no actions provided."""
    result = instrumented_agent.detect_divergence("I will help you", [])
    assert result is None


def test_detect_divergence_empty_speech(instrumented_agent):
    """Test divergence detection returns None when speech is empty."""
    from src.core.agent_schemas import ActionRecord

    action = ActionRecord(
        session_id=instrumented_agent._session_id,
        action_type="tool_call",
        description="Called dangerous_tool",
        target="system",
    )
    result = instrumented_agent.detect_divergence("", [action])
    assert result is None
...
def test_intercept_tool_call(instrumented_agent):
    ...
    instrumented_agent.register_tool(
        "login", login, "Login tool", sensitive_args=["password"]
    )

    result = instrumented_agent.intercept_tool_call(
        "login", username="admin", password="secret_password"
    )
...
def test_intercept_tool_call_positional_masking(instrumented_agent):
    ...
    instrumented_agent.register_tool(
        "connect", connect, "Connect", sensitive_args=["key"]
    )
...
def test_masking_mixed_args(instrumented_agent):
    ...
    instrumented_agent.wrap_tool_call(
        "api", api_call, "https://api.com", token="s3cr3t", retries=5
    )
...
def test_internal_memory_limit(instrumented_agent):
    ...
    # Fill memory
    instrumented_agent._internal_memory = {
        str(i): i for i in range(MAX_INTERNAL_MEMORY_KEYS)
    }
...
def test_memory_crud_internal(instrumented_agent):
    ...
    assert len(instrumented_agent.memory_accesses) == 1
    assert (
        instrumented_agent.memory_accesses[0].operation == MemoryAccessOperation.WRITE
    )
    ...
    assert (
        instrumented_agent.memory_accesses[2].operation == MemoryAccessOperation.DELETE
    )
...
def test_sensitive_memory_read(instrumented_agent):
    instrumented_agent.wrap_memory_access(
        MemoryAccessOperation.READ, "api_key", "secret"
    )
    ...
    instrumented_agent.wrap_memory_access(
        MemoryAccessOperation.READ, "my_password_field", "1234"
    )
...
def test_sensitive_memory_write(instrumented_agent):
    instrumented_agent.wrap_memory_access(
        MemoryAccessOperation.WRITE, "system_config", "danger"
    )
    event = instrumented_agent.memory_accesses[0]
    assert event.sensitive_detected is True

    instrumented_agent.wrap_memory_access(
        MemoryAccessOperation.WRITE, "__hidden", "danger"
    )
    event = instrumented_agent.memory_accesses[1]
    assert event.sensitive_detected is True

    instrumented_agent.wrap_memory_access(
        MemoryAccessOperation.WRITE, "user_data", "safe"
    )
    event = instrumented_agent.memory_accesses[2]
    assert event.sensitive_detected is False
